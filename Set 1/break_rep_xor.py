...
def bin_hamm_dist(a, b):
    ...
    if len(a) != len(b):
        raise ValueError("Hamming distances can only be computed for values of "
                         "equal length!\n len a = %d, len b = %d\na = %s\nb = %s"
                         % (len(a), len(b), a, b))
    dist = 0
...
def get_key_lengths(data, max_key_len=40):
    ...
    def calc_key_hamm(n):
        return mean([bin_hamm_dist(part[:n], part[n:]) / float(n)
...
if __name__ == "__main__":
    ...
    data = "This is a really great test message! You have no idea. Seriously."
    enc = rep_xor(data, "AM")
    crack_xor(enc)